...
-- notify_new_message — collapse to a single unread 'message' notification per
-- conversation for each recipient (except the sender). (20260921000000.)
...
-- notify_profile_changes — role change / (de)activation → the affected member.
-- (20260921000000.)
...
-- notify_group_member_added / _removed — group membership → the affected member.
-- Removal mirrors sync_group_member_removal's guard so a whole-group deletion
-- cascade does not notify every member. (20260921000000.)
...
-- notify_todo_events — assignment (to the assignee) and completion (to the
-- creator). New-task broadcast lives in notify_users_new_todo. (20260921000000.)
...
begin
  ...
  -- Keep the notification bell in sync with the Messages tab: reading a
  -- conversation clears its unread 'message' notification. (20260921000000.)
  update public.notifications
  set read_at = now()
...
-- Expanded notification coverage (20260921000000).
